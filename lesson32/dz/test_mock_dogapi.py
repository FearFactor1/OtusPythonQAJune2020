...
class TestClassBreeds:
    ...
    def test_LIST_ALL_BREEDS(self, api_client_mock, message, message2, message3):
#        res = requests.get('https://dog.ceo/api/breeds/list/all')
        res = requests_mock.Adapter().register_uri('GET', 'mock:https://dog.ceo/api/breeds/list/all', text='data')
        js = res.json()
